...
static long random_val(long min, long max)
{
    ...
    long result = min * (1-weight) + max * weight;
    return result;
}

/* Generate the integer values we'll use to test a function */
static long gen_vals(long test_vals[], long min, long max)
{
    long i;
    long test_count = 0;
...
static long test_0_arg(funct_t f, funct_t ft, char *name, int report)
{
    funct_t f1 = (funct_t) f;
    funct_t f1t = (funct_t) ft;
    long r = f1();
    long rt = f1t();
    long error =  (r != rt);

    if (error && report)
	printf("Test %s() failed.\n  Gives %ld[0x%016lx].  Should be %ld[0x%016lx]\n",
	       name, r, r, rt, rt);
    return error;
}

/* Test a function with one argument */
static long test_1_arg(funct_t f, funct_t ft, long arg1, char *name, int report)
{
    ...
    error = (r != rt);
    if (error && report)
	printf("Test %s(%ld[0x%016lx]) failed.\n  Gives %ld[0x%016lx].  Should be %ld[0x%016lx]\n",
	       name, arg1, arg1, r, r, rt, rt);
    return error;
}

/* Test a function with two arguments */
static long test_2_arg(funct_t f, funct_t ft, 
		      long arg1, long arg2, 
		      char *name, int report)
{
    ...
    if (error && report)
	printf(
	       "Test %s(%ld[0x%016lx],%ld[0x%016lx]) failed.\n  Gives %ld[0x%016lx].  Should be %ld[0x%016lx]\n",
	       name, arg1, arg1, arg2, arg2, r, r, rt, rt);
    return error;
}

/* Test a function with three arguments */
static long test_3_arg(funct_t f, funct_t ft,
	   ...
	   char *name, int report)
{
    ...
    if (error && report)
	printf(
	       "Test %s(%ld[0x%016lx],%ld[0x%016lx],%ld[0x%016lx]) failed.\n  Gives %ld[0x%016lx].  Should be %ld[0x%016lx]\n",
	       name, arg1, arg1, arg2, arg2, arg3, arg3, r, r, rt, rt);
    return error;
}

/* Test a function.  Return number of errors */
static long test_function(test_ptr t, int report) {
    long test_vals[3][TEST_COUNT];
    long test_counts[3];
    int errors = 0;
    int i;
    long a1, a2, a3;
...
static long run_tests(int report) {
    int i;
    long errors = 0;
    double points = 0.0;
